...
class Store {
  ...
  static init({context, child}) {
    ...
    return MultiProvider(
      providers: [
        ChangeNotifierProvider(builder: (_) => CountModel(0)), //在这提供provider
      ],
      child: child, //孩子
    );
  }
  ...
  //通过Consumer获取状态数据
  static Consumer<T> consumer<T>({builder, child}) {
...
class CounterButton extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    var increment = Store.value<CountModel>(context).increment; //获取数据
    var count = Store.value<CountModel>(context).count; //获取数据
...
class NextPage extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
//    var count = Store.value<CountModel>(context).count; //2; //获取数据
//    var app = MaterialApp(
//        title: 'Flutter Demo',
//        theme: ThemeData(
//          primarySwatch: Colors.blue,
...
//    return app;
    ...
    return MaterialApp(
        ...
        home: Scaffold(appBar: AppBar(
            title: Store.consumer<CountModel>(builder: (_, model, __) {
          Text("New Page,now count=${model.count}");
        }))));
  }
}
